...
class {%= prefix_caps %}_walker extends Walker_Nav_Menu {
...
class {%= prefix_caps %}_Excerpt {
	...
	public static function length($new_length = 55) {
		{%= prefix_caps %}_Excerpt::$length = $new_length;

		add_filter('excerpt_length', '{%= prefix_caps %}_Excerpt::new_length');

		{%= prefix_caps %}_Excerpt::output();
	}

	// Tells WP the new length
	public static function new_length() {
		if( isset({%= prefix_caps %}_Excerpt::$types[{%= prefix_caps %}_Excerpt::$length]) )
			return {%= prefix_caps %}_Excerpt::$types[{%= prefix_caps %}_Excerpt::$length];
		else
			return {%= prefix_caps %}_Excerpt::$length;
	}
